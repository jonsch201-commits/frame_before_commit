...
The half **points at files it does not contain.** The shipped `cfl/DERIVATION-LOG.md` (fourth derivation, 2026-09-13 07:47) reports 1,110 files content-scanned, 1,046 INCLUDED, 94 EXCLUDED, and carries one row per content exclusion; the 167 hook-state files withheld by a path prefix are cut before the scan and have no row in that log (`grep -c PATH-DENY-PREFIX` returns 0), so "every exclusion has a row" is true of content exclusions only. The dangling-citation count for the shipped tree is UNKNOWN until CFL re-runs its reference scan against this log; CFL's earlier figure for the superseded third derivation (01:37: 66 of 259 excluded files referred to by name from a shipping file, 859 references, the 259 = 167 hook-state + 92 content) is kept here as the last measured value and does not describe this tree. CFL's scratch re-run reports 249 and 52; run identity to be stated by CFL before those figures ship. Text reworded by Professional 17:1x on CFL's finding; original CFL sentence in git at d70059d.
