...
|---|---|---|---|---|---|---|
| `cfl/` | `N:/claude-cfl/clone` | 1,205 | 1,147 | 88 | 11.25 MB | 63344e09fe3980c9 (re-derived 01:1x with `scripts/lib/`, `scripts/graphrag/`, `scripts/tests/` added after the executability lint found imports the tree could not satisfy) |
...
`graph/index.sqlite` was built **from empty over this tree only**, with CFL's builder (`scripts/graphrag/build_index.py` @ fc2a771c; eighth build, over the re-derived `cfl/`):
...
| G1 graph coverage: every indexed path inside the tree; every .md on disk indexed | PASS (0 foreign; 1,002 of 1,002 under the halves, plus README and card; eighth build) |
